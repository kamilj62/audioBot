...
class CustomOpenAIModel:
    ...
    def __init__(
        self,
        api_key: str,
        organization: Optional[str] = Config.OPENAI_ORG_ID,
        project: Optional[str] = Config.OPENAI_PROJECT_ID,
        ...
        try:
            ...
            log(f"Failed to initialize OpenAI client: {e}", LogLevels.ON)
            raise
    ...
    def generate_stream(self, messages: list, config: Optional[ModelGenerationConfig]):
        """Send the message to the model and yield response chunks"""
        if not messages:
            raise ValueError("Messages are empty.")
        if not config:
            config = self.model_config
