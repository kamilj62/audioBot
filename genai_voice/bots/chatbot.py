...
class ChatBot:
    ...
    def __init__(
        ...
        if not context_file_path:
            context_file_path = PROMPTS_TO_CONTEXT_DATA_FILE.get(prompt)
            if not context_file_path:
                log(f"Warning: No predefined context file for prompt. Using generic fallback if available.", log_level=LogLevels.ON)
                context_file_path = "context.txt" # Default fallback
        ...
        match self.model_name:
            case Config.MODEL_GPT_TURBO_NAME:
                self.__client = CustomOpenAIModel(
    ...
    def _create_llm_history(self, llm_history: list):
        """Helper to build context messages from history"""
        if not llm_history:
            log("Empty history. Creating a state list to track histories.", log_level=LogLevels.ON)
            llm_history = []
        
        context = [self.llm_prompt]
        for interaction in llm_history:
            # Ensure interaction is a pair (user, assistant)
    ...
    def respond(self, prompt, llm_history: list = None):
        ...
        """
        context = self._create_llm_history(llm_history)
    ...
    def respond_stream(self, prompt, history, enable_voice=True):
        """Streaming version of respond"""
        context = self._create_llm_history(history)
        context.append({"role": "user", "content": f"{prompt}"})
        
        full_response = ""
        # Yield each chunk to the UI
        for chunk in self.__client.generate_stream(context, self.__client.config):
            full_response += chunk
            yield chunk
            
        # Play audio at the end if enabled
        if self.__enable_speakers and enable_voice:
            if self.__threaded:
                threading.Thread(
                    target=self.audio.communicate, args=(full_response,)
                ).start()
            else:
                self.audio.communicate(full_response)
    ...
    def get_prompt_from_gradio_audio(self, audio, use_api=True):
        ...
        use_api: Whether to use OpenAI API for transcription (saves RAM)
        """
        log(f"Getting prompt from audio device: {audio}")
        if not audio:
            return None
        return self.audio.get_prompt_from_gradio_audio(audio, use_api=use_api)
